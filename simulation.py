...
class Simulation:
    ...
    def __simulate_one_week__(self):
        ...
        for member_doc in members:
            ...
            if get_random_number() < member.get_recruit_probability():
                ...
                print('Member {} will try to recruit investor {}'.format(member.id, candidate.id))
                # Same problem with 3.c of the document
                if get_random_number() < candidate.get_accept_probability():
                    print('Investor {} will join the program invited by member {}'.format(candidate.id, member.id))
                    member.money += 100
                    self.db_controller.add_member(candidate.id, member.id, self.current_week)
                else:
                    print('Investor {} will not join the program'.format(candidate.id))
            else:
                print('Member {} will not recruit a new investor'.format(member.id))
            lasted_weeks = self.current_week - member.week_joined
            if (member.money < 500) & (lasted_weeks >= member.get_max_weeks()):
                print('Member {} did not recover the investment in {} weeks and will be eliminated from the program with MM${}.'.format(member.id, lasted_weeks, member.money))
    ...
    def run(self):
        ...
        print('The program ended in {} weeks and the mummy got MM${}'.format(self.current_week, mummy_money))
